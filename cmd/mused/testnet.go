...
func testnetCmd(ctx *server.Context, cdc *codec.Codec,
	...
	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a mused testnet",
		Long: `testnet will create "v" number of directories and populate each with
necessary files (private validator, genesis, config, etc.).
Note, strict routability for addresses is turned off in the config file.
Example:
	mused testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if genAccIterator == nil {
				genAccIterator = GenesisAccountIterator{}
			}
	...
	cmd.Flags().String(flagNodeDaemonHome, "mused",
		"Home directory of the node's daemon configuration")
	cmd.Flags().String(flagNodeCLIHome, "musecli",
...
func InitTestnet(cmd *cobra.Command, config *tmconfig.Config, cdc *codec.Codec,
	...
	museConfig := srvconfig.DefaultConfig()
	museConfig.MinGasPrices = minGasPrices

	//nolint:prealloc
	var (
		genAccounts []authexported.Account
		genFiles    []string
	)
	...
	for i := 0; i < numValidators; i++ {
		...
		keyPass := client.DefaultKeyPass //果然，密码硬编码了，不用在shell交互了
		...
		coins := sdk.Coins{
			sdk.NewCoin(fmt.Sprintf("%stoken", nodeDirName), accTokens), //1个节点1种币？
		...
		// TODO: Rename config file to server.toml as it's not particular to Muse
		// (REF: https://github.com/cosmos/cosmos-sdk/issues/4125).
		museConfigFilePath := filepath.Join(nodeDir, "config/mused.toml")
		srvconfig.WriteConfigFile(museConfigFilePath, museConfig)
	}
...
type genesisState struct {
	Params    auth.Params      `json:"params" yaml:"params"`
	Accounts []authexported.Account `json:"accounts" yaml:"accounts"`
}

func initGenFiles(
	cdc *codec.Codec, mbm module.BasicManager, chainID string,
	genAccounts []authexported.Account, genFiles []string, numValidators int,
) error {
...
func (GenesisAccountIterator) IterateGenesisAccounts(
	cdc *codec.Codec, appGenesis map[string]json.RawMessage, cb func(authexported.Account) (stop bool),
) {
